...
def location(relative_path: str = "main.go", start_line: int = 0) -> SourceLocation:
...
def make_type(
    name: str, relative_path: str = "main.go", *, underlying_kind: str = "struct"
) -> TypeDeclaration:
    fid = file_id(REPOSITORY_ID, Path(relative_path))
...
def make_import(
    name: str, relative_path: str = "main.go", *, occurrence: int = 0
) -> ImportDeclaration:
    fid = file_id(REPOSITORY_ID, Path(relative_path))
    return ImportDeclaration(
        id=declaration_id(fid, "import", name, occurrence),
        name=name,
        location=location(relative_path),
        import_path=name,
    )
